...
const Expenses = () => {
  ...
  const getAllExpenses = async function () {
    ...
    try {
      ...
      if (res.data.status !== 'FAILED') {
        setExpenses(res.data);  // Set the fetched user data in the state
        setLoading(false);
  ...
  const handleButtonClick = () => {
    ...
    navigate('/expenses/new', { state: { selectedCollection } });
  };

  const handleRowClick = (args) => {
    // Log the row data to the console
    console.log(args.data);
    navigate('/expenses/details', { state: { data: args.data, selectedCollection } });
  };

 
  const columns = [
 
    { field: 'personName', headerText: 'شخص نوم', width: 100 ,textAlign: 'Center',},
    { field: 'expenseDate', headerText: 'نیټه', width: 100,textAlign: 'Center', },
    { field: 'amount', headerText: 'مقدار په ډالرو', width: 100,textAlign: 'Center',},
    { field: 'reason', headerText: 'لګښت دلیل', width: 150 ,textAlign: 'Center',},
