...
const Purchases = () => {
  ...
  const getAllPurchases = async function () {
    ...
    try {
      ...
      if (res.data.status !== 'FAILED') {
        setIsLoading(false);
        setPurchases(res.data);  // Set the fetched user data in the state
        setLoading(false);
  ...
  const handleButtonClick = () => {
    ...
    navigate('/purchases/new', { state: { selectedCollection } });
  };
  ...
  const handleRowClick = (args) => {
    ...
    navigate('/purchases/details', { state: { data: args.data, selectedCollection } });
  };
 
  const columns = [
 
    { field: 'purchaseDate', headerText: 'خرید نیټه', width: 100 ,textAlign: 'Center',},
    { field: 'supplier', headerText: 'عرضه کوونکی', width: 100,textAlign: 'Center', },
