...
const CustomerAccountOld = ({accountData}) => {
    const SERVER_PATH = process.env.REACT_APP_SERVER_PATH;
    // Get the navigate function
    
    const customer_id = accountData?._id;
    const customer_name=accountData?.customerName;
    const customer_mobile=accountData?.contactNumber
   ...
   try {
    ...
    try {
      ...
      setCustomerPayments(res.data.data);
      } else {
        setIsLoading(false)
        console.log(res);
      }
    ...
    const columns = [
      ...
      { field: 'transactionType', headerText: 'ډول', width: 150,textAlign: 'Center',},
      // ... other columns
    ]; 
...
export default CustomerAccountOld;