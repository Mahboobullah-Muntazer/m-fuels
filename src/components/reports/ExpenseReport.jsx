...
const ExpenseReport = () => {
  ...
  const calculateSums = () => {
    ...
    expenses.forEach((expense) => {
      totalPriceSum += expense.expense.amount;
  ...
  const getAllExpenses = async function () {
      ...
      }else
    {
      ...
      try {
        ...
        if (res.data.status !== 'FAILED') {
          const sortedExpenses = res.data.data.sort((a, b) => new Date(b.expenseDate) - new Date(a.expenseDate));
...
<div id="expensesTable" className='mt-5'>
<div className='flex w-full justify-between px-5 bg-slate-300 rounded-sm'>  <h2 className='font-bold text-lg mt-5 mb-5'>Expense Report / لګښت ریپورټ</h2>
<h2 className='font-bold text-lg mt-5 mb-5'>میاشت او کال : {reportCollection?.collectionName} </h2>
</div><table className="min-w-full border border-gray-300">
           <thead>
             <tr>
          
               
           
             <th className="py-2 px-4 border-b">مصرف نیټه</th>
      ...
      <td className="py-2 px-4 border-b text-center">{expense?.expense?.expenseDate}</td>
                 <td className="py-2 px-4 border-b text-center">{expense?.expense?.amount}</td>
                 <td className="py-2 px-4 border-b text-center">{expense?.expense?.reason}</td>
                 <td className="py-2 px-4 border-b text-center">{expense?.expense?.personName}</td>
