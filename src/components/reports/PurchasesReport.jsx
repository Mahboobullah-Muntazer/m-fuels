...
const PurchasesReport = () => {
  ...
  const calculateSums = () => {
    ...
    purchases.forEach((purchase) => {
      totalPriceSum += purchase.purchase.totalPrice;
      quantityInLitersSum += purchase.purchase.quantityInLiters;
      quantityInTonsSum += purchase.purchase.quantityInTons;
    });
  ...
  const getAllPurchases = async function () {
    ...
    try {
      ...
      if (res.data.status !== 'FAILED') {
        ...
        const sortedPurchases = res.data.data.sort((a, b) => new Date(b.purchaseDate) - new Date(a.purchaseDate));
...
<div id="purchasesTable" className='mt-5'>
<div className='flex w-full justify-between px-5 bg-slate-300 rounded-sm'>  <h2 className='font-bold text-lg mt-5 mb-5'>Purchase Report / خرید ریپورټ</h2>
...
<table className="min-w-full border-collapse border-gray-300">
           <thead>
             ...
             <th className="py-2 px-4 border border-black">خرید نیټه</th>
               <th className="py-2 px-4 border border-black">عرضه کوونکی</th>
               ...
               <th className="py-2 px-4 border border-black">نمبر پلیټ</th>
      ...
      <td className="py-2 px-4 border border-black text-center">{purchase?.purchase?.purchaseDate}</td>
              <td className="py-2 px-4 border border-black text-center">{purchase?.purchase?.supplier}</td>
              <td className="py-2 px-4 border border-black text-center">{purchase?.purchase?.fuelType}</td>
                 <td className="py-2 px-4 border border-black text-center">{purchase?.purchase?.totalPrice}</td>
                 <td className="py-2 px-4 border border-black text-center">{purchase?.purchase?.quantityInLiters}</td>
                 <td className="py-2 px-4 border border-black text-center">{purchase?.purchase?.quantityInTons}</td>
                 <td className="py-2 px-4 border border-black text-center">{purchase?.purchase?.driverName}</td>
                 <td className="py-2 px-4 border border-black text-center">{purchase?.purchase?.plateNumber}</td>
             ...
             <td colSpan="3"></td>
               <td className="py-2 px-4  bg-gray-300 font-bold text-center">{totalPriceSum}</td>
               <td className="py-2 px-4  bg-gray-300 font-bold text-center">{quantityInLitersSum}</td>
               <td className="py-2 px-4  bg-gray-300 font-bold text-center">{quantityInTonsSum}</td>
               <td colSpan="1" className="py-2 px-4 bg-gray-300 font-bold text-center"></td>
              
           
              
               <td colSpan="1" className="py-2   bg-gray-300 px-4 font-bold">مجموعه</td>
