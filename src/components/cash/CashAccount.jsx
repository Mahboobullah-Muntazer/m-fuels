...
const CashAccount = ({ accountData }) => {
  ...
  const handleTransactionFetch=async()=>{
   ...
   {
    toast.warning("لطفا لمړی میاشت او کال انتخاب کړی")

   }else
   {
    
    const getTransaction = `${SERVER_PATH}api/actions/getTransactionOfCashPayment/${selectedCollection._id}`;
    ...
    <div className="bg-white md:mt-20 w-full mx-auto">
      ...
      <div className="shadow-sm shadow-teal-500 p-4 bg-white">
        <div className="text-xl font-bold mb-4 mt-5 flex justify-between">
          <h1>Cash Account / نقده حساب</h1>
         
        </div>
        <div className="grid grid-cols-1 md:grid-cols-1 gap-4 mb-6 border border-teal-500 p-4">
        
          <div className={`p-4 ${cash?.balance >= 0 ? 'bg-blue-500' : 'bg-red-500'} rounded-lg shadow w-full md:w-72 h-60`}>
            <div className="text-white text-xl font-semibold tracking-wider">Balance</div>
            <div className="text-white"><strong>{cash?.balance}</strong></div>
          </div>
        </div>
      ...
      <div className={`overflow-hidden transition-all duration-500 ${viewDetails ? 'max-h-screen opacity-100' : 'max-h-0 opacity-0'}`}>
        {viewDetails &&
